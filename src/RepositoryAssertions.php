...
{
    private static string $countWithoutCriteriaDeprecationMessagePattern = 'Passing the message to %s() as second parameter is deprecated. Use third parameter as message.';
    ...
    public function count(int $expectedCount, array|string $criteria = [], string $message = 'Expected count of {entity} repository ({actual}) to be {expected}.'): self
    {
        ...
        Assert::that($this->repository->countWithCriteria(is_array($criteria) ? $criteria : []))
